...
public class adVideoController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (action == null) {
        ...
        } else if (action.equals("edit")) { // Lấy thông tin video theo id và
         //hiển thị form chỉnh sửa
            Integer id = Integer.parseInt(req.getParameter("id"));

            Video video = videoService.findById(id);
            req.setAttribute("video", video);
         req.getRequestDispatcher("/WEB-INF/views/videos/edit.jsp").forward(req,resp);
        } else if (action.equals("delete")) { // Xóa video theo id và chuyển
         //hướng về trang danh sách video
            Integer id = Integer.parseInt(req.getParameter("id"));
            videoService.delete(String.valueOf(id));
         ...
         (action.equals("detail")) { // Lấy thông tin video theo id và hiển thị tran chi tiết
            ...
            Video video = videoService.findById(id);
            req.setAttribute("video", video);
         req.getRequestDispatcher("/WEB-INF/views/videos/detail.jsp").forward(req,resp);
        }
    }
      @Override
...
//            // Chỉnh sửa video từ form và chuyển hướng về trang
